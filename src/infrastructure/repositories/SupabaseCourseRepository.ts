...
export class SupabaseCourseRepository implements ICourseRepository {
	...
	async createDraftVersion(
		...
		const payload = await this.buildVersionPayload(
			supabase,
			input.courseId,
			{
				title: input.title,
	...
	async publishCourseVersion(
		...
		if (versionError) {
			throw new Error(
				versionError.message || "Error al publicar la versión"
			);
		}
	...
	async archiveCourseVersion(
		...
		if (versionError) {
			throw new Error(
				versionError.message || "Error al archivar la versión"
			);
		}
	...
	async getCourseTeachers(courseId: string): Promise<string[]> {
		...
		if (versionsError) {
			throw new Error(versionsError.message || "Error al obtener versiones del curso");
		}
		...
		if (groupsError) {
			throw new Error(groupsError.message || "Error al obtener profesores del curso");
		}
	...
	async reorderTopics(
		...
		const whenClauses = order
			.map(({ topicId, orderIndex }) => `WHEN id = '${topicId}' THEN ${orderIndex}`)
			.join(' ');
		...
		if (error) {
			...
			for (const { topicId, orderIndex } of order) {
				...
				if (updateError) {
					throw new Error(updateError.message || `Error al reordenar tópico ${topicId}`);
				}
			}
		}
	}
	...
	async reorderResources(
		...
		console.log('🔄 Reordenando recursos:', {
			topicId,
			count: order.length,
			newOrder: order.map(o => `${o.resourceId.slice(0,8)}... → ${o.orderIndex}`).join(', ')
		});
		...
		if (error) {
			// If the RPC function doesn't exist, try individual updates as fallback
			console.warn('RPC reorder_resources_batch no disponible, usando fallback');
			...
			for (const { resourceId, orderIndex } of order) {
				...
				if (updateError) {
					throw new Error(updateError.message || `Error al reordenar recurso ${resourceId}`);
				}
			}
		}
	}
	...
	async listGroups(courseVersionId: string): Promise<CourseGroupEntity[]> {
		...
		if (error) {
			throw new Error(
				error.message || "Error al obtener los grupos del curso"
			);
		}
	...
	async addStudentToGroup(
		...
		if (error) {
			throw new Error(
				error.message || "Error al agregar estudiante al grupo"
			);
		}
	...
	private async getVersionsByCourseIds(
		...
		if (error) {
			throw new Error(
				error.message || "Error al obtener versiones del curso"
			);
		}
