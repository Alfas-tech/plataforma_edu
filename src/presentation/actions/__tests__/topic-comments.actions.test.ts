...
import { getTopicComment, upsertTopicComment, createCommentResponse } from '../topic-comments.actions';
...
describe('topic-comments.actions', () => {
  ...
  test('getTopicComment returns comment and responses when present', async () => {
    const fakeComment = { id: 'c1', topic_id: 't1', content: 'hola', created_at: 'now', updated_at: 'now' };
    const fakeResponses = [{ id: 'r1', comment_id: 'c1', content: 'resp', created_at: 'now' }];
  ...
  test('getTopicComment returns empty when no comment found', async () => {
    ...
    const topicEq: any = jest.fn().mockReturnThis();
    const topicSingle: any = jest.fn();
    (topicSingle as any).mockResolvedValue({ data: null, error: { code: 'PGRST116', message: 'No rows' } });
  ...
  test('upsertTopicComment rejects when not authenticated or not editor', async () => {
    ...
    mockGetCurrentProfile.mockResolvedValueOnce({ profile: { id: 'u1', role: 'student' } });
  ...
  test('upsertTopicComment updates existing comment when present', async () => {
    mockGetCurrentProfile.mockResolvedValueOnce({ profile: { id: 'ed1', role: 'editor' } });
    ...
    mockFrom.mockImplementation((table: any) => {
      if (table === 'topic_comments') {
        return { select: topicSelect, eq: topicEq, single: topicSingle, update: topicUpdate } as any;
      }
      return {} as any;
    });

    const res = await upsertTopicComment({ topic_id: 't1', content: 'updated' });
  ...
  test('createCommentResponse inserts and returns success', async () => {
    ...
    const res = await createCommentResponse({ comment_id: 'c1', content: 'ok' });
    expect(res).toHaveProperty('success');
    expect((res as any).success).toBe(true);
  });
