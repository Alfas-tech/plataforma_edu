...
import { LogOut, ArrowLeft, GraduationCap, Users } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import { TeacherAssignmentClient } from "./components/TeacherAssignmentClient";
...
export default async function CourseTeachersPage({ params }: PageProps) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50">
        <div className="container mx-auto px-4 py-8">
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
            <p>Error al cargar curso: {courseResult.error}</p>
          </div>
        </div>
      </div>
    );
  ...
  if ("error" in usersResult) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50">
        <div className="container mx-auto px-4 py-8">
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
            <p>Error al cargar docentes: {usersResult.error}</p>
          </div>
        </div>
      </div>
    );
